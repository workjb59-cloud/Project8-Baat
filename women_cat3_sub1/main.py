...
# Hardcoded subcategory URLs for Niche Perfumes
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/women/fragrances/niche-perfumes-1/l/",
]
...
class BoutiqaatDataPipeline:
    ...
    def run(self):
        ...
        logger.info("=" * 80)
        logger.info("Starting Boutiqaat Data Pipeline - Niche Perfumes (Sequential)")
        logger.info("=" * 80)
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/niche-perfumes/images/"
                f"{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/niche-perfumes/excel-files"
            )
